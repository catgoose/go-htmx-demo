...
func (ar *appRoutes) initPwaRoutes() {
	...
	ar.e.POST("/pwa/inspection", func(c echo.Context) error {
		return handler.RenderComponent(c, views.PwaFormSuccess("Site inspection saved successfully."))
	})
	ar.e.POST("/pwa/report", func(c echo.Context) error {
		return handler.RenderComponent(c, views.PwaFormSuccess("Field report saved successfully."))
	})
	ar.e.POST("/pwa/notes", func(c echo.Context) error {
		return handler.RenderComponent(c, views.PwaFormSuccess("Notes saved successfully."))
	})
}
