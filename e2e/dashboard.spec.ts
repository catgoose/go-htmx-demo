...
test.describe("Dashboard", () => {
  ...
  test("inventory card links to demo pages", async ({ page }) => {
    await navigateTo(page, "/dashboard");
    await expect(page.locator('a[href="/demo/inventory"]').first()).toBeVisible();
    await expect(page.locator('a[href="/demo/catalog"]')).toBeVisible();
    await expect(page.locator('a[href="/demo/bulk"]')).toBeVisible();
  });
  ...
  test("clicking inventory link navigates to inventory page", async ({ page }) => {
    await navigateTo(page, "/dashboard");
    await page.locator('a[href="/demo/inventory"]').first().click();
